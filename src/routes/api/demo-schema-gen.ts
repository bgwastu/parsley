// ABOUTME: Demo schema generation API endpoint for server-side AI requests
// ABOUTME: Generates schemas using server-managed API key with rate limiting
...
const DemoSchemaGenRequestSchema = z.object({
...
function parseDemoFormData(formData: FormData) {
	...
	if (!fileEntry || !outputFormatStr) {
		throw new Error("Missing required fields: file and outputFormat are required");
	}

	if (typeof outputFormatStr !== "string") {
	...
	return DemoSchemaGenRequestSchema.parse({
		file: fileEntry,
...
async function handleDemoSchemaGen(
	formData: FormData,
): Promise<unknown | ApiErrorResponse> {
	try {
		const validatedData = parseDemoFormData(formData);
		const { file, outputFormat, pdfPassword, pdfRange } = validatedData;
		...
		console.error("Demo schema generation error:", error);
...
export const Route = createFileRoute("/api/demo-schema-gen")({
	server: {
		handlers: {
			POST: async ({ request }) => {
						...
						return json(
							{
								...
								error: {
									message:
										"Demo schema generation limit reached (10/day). Add your own API key in Settings for unlimited access.",
				...
				const result = await handleDemoSchemaGen(formData);
				const isError =
					result &&
